...
```{r}
O14<-mtransp%>%left_join(disch)%>%mutate(Date=as.Date( paste(Month, Day , Year , sep = "." )  , format = "%m.%d.%Y"), dec.date=decimal_date(Date), dec.during=dec.date-year(Date))
...
```{r}
modO14<-gamm(logTN~s(dec.date)+logq,correlation=corCAR1(form = ~dec.date),method="REML", data=O14)
summary(modO14$gam)

modO14_notrend<-gls(logTN~logq, correlation=corCAR1(form = ~dec.date),data=O14, na.action=na.omit)
...
```{r}
modO14_trend_only<-gamm(logTN~s(dec.date),correlation=corCAR1(form = ~dec.date),method="REML", data=O14)
terms<-predict.gam(modO14_trend_only$gam, type="terms", newdata=O14, se.fit=TRUE)

O14$fit<-terms$fit[,1]+attr(terms, "constant")

ggplot(O14, aes(dec.date, logTN,fit)) +
...
```{r}
modO14_GAM_N<-gamm(logTN~s(dec.date)+s(logq),correlation=corCAR1(form = ~dec.date),method="REML", data=O14)
...
O14$fit_nlin<-terms$fit[,1]+attr(terms, "constant")+ mean(O14$logq, na.rm=TRUE)*summary(modO14$gam)$p.coeff[2]
...
```{r}
O14%>%
  ggplot(aes(x=logq, y=logTN, Line_lower,Line_upper))+
  geom_point()+
...
```{r}
modO14_GAM_NS<-gamm(logTN~s(dec.date)+seas+s(logq, by=seas),correlation=corCAR1(form = ~dec.date),method="REML", data=O14)
...
O14$fit_seas<-terms1$fit[,2]+attr(terms1, "constant")+ mean(O14$logq, na.rm=TRUE)*summary(modO14$gam)$p.coeff[2]
O14$sereg<-terms1$se.fit[,4]
O14$line<-terms1$fit[,4]+mean(terms1$fit[,2], na.rm=TRUE)+attr(terms1, "constant")
...
```{r}
O14$fit_seas<-terms1$fit[,1]+attr(terms1, "constant")+ mean(O14$logq, na.rm=TRUE)*summary(modO14$gam)$p.coeff[2]
O14$sereg<-terms1$se.fit[,5]
O14$line<-terms1$fit[,5]+mean(terms1$fit[,2], na.rm=TRUE)+attr(terms, "constant")
...
O14$line<-terms1$fit[,6]+mean(terms1$fit[,2], na.rm=TRUE)+attr(terms1, "constant")
...
O14$line<-terms1$fit[,3]+mean(terms1$fit[,2], na.rm=TRUE)+attr(terms1, "constant")
