...
Fyrismod_constant_withtrend<-gamm(log_TP~s(dec.date)+log_q, correlation=corCAR1(form = ~dec.date),method="REML", data=Fyris)
...
```{r}
modFyris_GAM_T<-gamm(log_TP~s(dec.date, k=40) +s(dec.date, by=log_q),data=Fyris, correlation=corCAR1(form=~dec.date), method="REML")

terms1d<-predict.gam(modFyris_GAM_T$gam, type="terms", se.fit=TRUE)

Fyris$fit<-terms1d$fit[,1]+attr(terms1d, "constant")+terms1d$fit[,2]/Fyris$log_q*mean(Fyris$log_q, na.rm=TRUE)
Fyris$fit.upper<-Fyris$fit+1.96*terms1d$se.fit[,1]
Fyris$fit.lower<-Fyris$fit-1.96*terms1d$se.fit[,1]
Fyris$coeff<-terms1d$fit[,2]/Fyris$log_q
Fyris$coeff.upper<-Fyris$coeff+1.96*terms1d$se.fit[,2]/Fyris$log_q
Fyris$coeff.lower<-Fyris$coeff-1.96*terms1d$se.fit[,2]/Fyris$log_q
```

Plot c-q slopes.
```{r}
ggplot(Fyris, aes(dec.date, coeff,coeff.lower,coeff.upper)) +
